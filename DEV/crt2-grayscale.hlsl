...
#define TINT_DEV                float3(1.1, 1.1, 1.1)
...
//-- Shader goes here --//
  ...
  // Bezel
  if(uv.x <  -0.015 || uv.y <  -0.015) return float4(0.04, 0.04, 0.04, 1.0);
  if(uv.x >   1.015 || uv.y >   1.015) return float4(0.04, 0.04, 0.04, 1.0);
  // Screen Border
